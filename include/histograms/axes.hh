...
struct poly_axis_base: axis_base<Edge> {
  ...
  virtual size_type find_bin(edge_type x) const = 0;
...
public:
  ...
  size_type nedges() const noexcept { return _edges.size(); }
  size_type nbins() const noexcept { return _edges.size()-1; }

  edge_type edge(size_type i) const noexcept { return _edges[i]; }
  edge_type operator[](size_type i) const noexcept { return edge(i); }
  ...
  edge_type lower(size_type i) const noexcept {
    return i==0 ? lowest : i>nedges()+1 ? highest : edge(i-1);
  }
  edge_type upper(size_type i) const noexcept {
  ...
  template <typename T>
  size_type operator()(const T& x) const noexcept { return find_bin(x); }
...
public:
  ...
  size_type nedges() const noexcept { return _nbins+1; }
  size_type nbins() const noexcept { return _nbins; }

  edge_type edge(size_type i) const noexcept {
  ...
  size_type find_bin(const T& x) const noexcept {
    ...
    return size_type(_nbins*(x-_min)/(_max-_min)) + 1;
  }
  template <typename T>
  size_type operator()(const T& x) const noexcept { return find_bin(x); }
};
...
public:
  ...
  size_type nedges() const noexcept { return _ref->nedges(); }
  size_type nbins() const noexcept { return _ref->nbins(); }

  edge_type edge(size_type i) const noexcept { return _ref->edge(i); }
  edge_type operator[](size_type i) const noexcept { return edge(i); }

  edge_type min() const noexcept { return _ref->min(); }
  edge_type max() const noexcept { return _ref->max(); }

  edge_type lower(size_type i) const noexcept { return _ref->lower(i); }
  edge_type upper(size_type i) const noexcept { return _ref->upper(i); }

  template <typename T>
  size_type find_bin(const T& x) const noexcept { return _ref->find_bin(x); }
  template <typename T>
  size_type operator()(const T& x) const noexcept { return find_bin(x); }
};
